...
#message_lines = list(map(lambda x: x + "\n", message_lines))
message = list(zip(cipher_lines, message_lines))

def printMult(text, i):
    if i < len(text):
        print(f"C:{i} ", text[i][0])
        print(f"P:{i} ", text[i][1])
        return printMult(text, i+1 )
    else:
        pass
